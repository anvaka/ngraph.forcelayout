...
export default function generateQuadTreeFunction(dimension) {
...
${runRecursiveOnChildren()}
  ...
  function insertBodies(graph, settings) {
    ${pattern('let {var}min = Number.MAX_VALUE;', {indent: 4})}
    ${pattern('let {var}max = Number.MIN_VALUE;', {indent: 4})}
  ...
  function insert(newBody) {
    ...
    while (!insertStack.isEmpty()) {
      const stackItem = insertStack.pop();
...
`;
  ...
  function assignInsertionQuadIndex(indentCount) {
    const insertionCode = [];
    const indent = Array(indentCount + 1).join(' ');
...
export function isSamePosition(dimension) {
  const pattern = createPatternBuilder(dimension);
  return `
...
export function setChildBodyCode(dimension) {
  const quadCount = Math.pow(2, dimension);
  return `
...
export function getChildBodyCode(dimension) {
...
export function getQuadNodeCode(dimension) {
  const pattern = createPatternBuilder(dimension);
  const quadCount = Math.pow(2, dimension);
  const quadNodeCode = `
function QuadNode() {
...
export function getInsertStackCode() {
  return `
/**
