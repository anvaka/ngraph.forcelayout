...
export default function createPhysicsSimulator(graph, settings) {
  settings = merge(settings, {
    ...
    /**
     * Graphology Graph option.
     * Should the graph allow self-loops? Boolean
     * https://graphology.github.io/instantiation.html#options
     */
    allowSelfLoops: true,

    /**
     * Graphology Graph option.
     * Should the graph allow parallel edges? Boolean
     * https://graphology.github.io/instantiation.html#options
     */
    multi: true,
    ...
    /**
     * Graphology Graph option.
     * Type of the graph. One of 'directed', 'undirected' or 'mixed'.
     * https://graphology.github.io/instantiation.html#options
     */
    type: 'directed',
  });
  ...
  if (!graph) {
    graph = new Graph({ 
      allowSelfLoops: settings.allowSelfLoops, 
  ...
  const publicApi = {
    ...
    /**
     * Returns the number of bodies in the graph
     */
    countBodies: countBodies,

    /**
     * Returns the number of springs in the graph
     */
    countSprings: countSprings,
    ...
    /**
     * Creates body at given position
     *
     * @param {Object} pos position of a body
     *
     * @returns {ngraph.physics.primitives.Body} created body
     */
    ...
    /**
     * Adds a body to the given node within the graph
     * 
     * @param {any} nodeId graphology node
     * @param {ngraph.physics.primitives.Body} body the body to add to the node
     * @returns {boolean} returns true if successfully added body to node
     */
    addBody: function (nodeId, body) {
      if (!nodeId) {
        throw new Error("Node must be provided to addBody");
      };
      if (!body) {
      ...
      if (graph.getNodeAttribute(nodeId, settings.body)) {
        throw new Error("Node already has body");
      }
      graph.setNodeAttribute(nodeId, settings.body, body);
      return true;
    },
    ...
    addSpring: function (edgeId, spring) {
      if (!graph.hasEdge(edgeId)) {
        throw new Error("Cannot create spring on nonexistant edge");
      }
      if (!spring) {
  ...
  function setDimensions(dimensions) {
    ...
    bounds = factory.createBounds(graph, settings, random);
    springForce = factory.createSpringForce(settings, random);
    dragForce = factory.createDragForce(settings);

    graph.forEachNode((nodeId, attributes) => {
      if (!attributes[settings.body]) return;
      const body = attributes[settings.body];
      body.pos = factory.createVector(body.pos);
      body.force = factory.createVector(body.force);
      body.velocity = factory.createVector(body.velocity);
      graph.setNodeAttribute(nodeId, settings.body, body);
    });
  }
  ...
  function getBody(nodeId) {
    // untouched
    return graph.getNodeAttribute(nodeId, settings.body);
  }

  function countBodies() {
    const numBodies = graph.reduceNodes(((accum, node, attr) => {
      if (attr[settings.body]) {
        accum += 1;
      }
      return accum;
    }), 0);
  ...
  function countSprings() {
    const numSprings = graph.reduceEdges(((accum, edge, attr) => {
      if (attr[settings.spring]) {
        accum += 1;
      }
      return accum;
    }), 0);
  ...
  function nbodyForce() {
    quadTree.insertBodies(graph, settings);
    graph.forEachNode((nodeId, attrs) => {
      if (!attrs[settings.body]) return;
      const body = attrs[settings.body];
      if (!body.isPinned) {
        body.reset();
  ...
  function getNodeAttribute(nodeId, attr) {
    if (!graph.hasNodeAttribute(nodeId, attr)) {
      return false;
    }
    const value = graph.getNodeAttribute(nodeId, attr);
    return value;
  }
