...
const srcCode = staticCompiled.toString().split('\n').slice(1, -1).join('\n');
const dynamicCompiled = compile(srcCode);
resetBodies();
...
function staticCompiled(bodyCollection, timeStep, adaptiveTimeStepWeight) {
  let tx = 0, ty = 0;
  const  max = bodyCollection.length;
  ...
  for (let i = 0; i < max; ++i) {
    const body = bodyCollection[i];
    if (adaptiveTimeStepWeight && body.springCount) {
      timeStep = (adaptiveTimeStepWeight * body.springLength/body.springCount);
    }

    const coefficient = timeStep / body.mass;
    ...
    const dx = timeStep * body.velocity.x;
    const dy = timeStep * body.velocity.y;
